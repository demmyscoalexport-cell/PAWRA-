/**
 * @file Card.tsx
 * @description Minimal surface cards — no heavy elevation.
 */
...
const BASE = 'rounded-md bg-surface overflow-hidden';
...
export function Card({
    ...
    return (
      <article className={`${BASE} ${className}`}>
        ...
        <div className="space-y-3 bg-surface p-6">
          {badge && <Badge type={badge} />}
          {title && <h3 className="font-sans text-display-m text-text-primary">{title}</h3>}
  ...
  if (variant === 'stat') {
    return (
      <article className={`${BASE} border border-border-subtle bg-page-bg p-6 text-center ${className}`}>
        {value && <p className="font-mono text-display-m text-text-primary">{value}</p>}
  ...
  if (variant === 'testimonial') {
    return (
      <article className={`${BASE} border-l border-border-subtle p-6 ${className}`}>
        {quote && <p className="font-sans text-heading-s text-text-primary">&ldquo;{quote}&rdquo;</p>}
  ...
  return (
    <article className={`${BASE} bg-inverse p-6 text-text-inverse ${className}`}>
      {badge && <Badge type={badge} />}
      {title && <h3 className="mt-3 font-sans text-heading-m text-text-inverse">{title}</h3>}
